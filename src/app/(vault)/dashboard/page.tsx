  ...
  return (
    <div className="space-y-4 md:space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6 auto-rows-min">
        ...
        {/* Activity Timeline */}
        <div className="md:col-span-2 xl:col-span-2 md:row-span-2">
        ...
        <div className="md:col-span-1 xl:col-span-1 bg-white rounded-xl md:rounded-[14px] p-4 md:p-6 shadow-[0_4px_24px_rgba(46,125,50,0.08)] flex flex-col justify-between">
          <ScrollReveal delay={0.5} className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-3 md:mb-4">
              <h3 className="font-heading font-bold text-base md:text-lg text-dark leading-tight">Goals</h3>
              <span className="text-[10px] md:text-xs font-bold bg-primary/10 text-primary px-2 py-0.5 md:py-1 rounded-md">{activeGoalsCount} Active</span>
            </div>
            
            {memberGoals.length > 0 ? (
              <div className="space-y-2 md:space-y-3 mb-3 md:mb-4">
                <p className="text-xs md:text-sm text-text-primary font-medium truncate">{memberGoals[0].title}</p>
                <div className="h-1.5 md:h-2 w-full bg-surface-alt rounded-full overflow-hidden">
                  <div className="h-full bg-primary rounded-full w-[45%]"></div>
                </div>
              </div>
            ) : (
              <p className="text-xs md:text-sm text-text-muted mb-3 md:mb-4">No active goals.</p>
            )}
            
            <a href="/goals" className="text-xs md:text-sm font-medium text-secondary hover:underline mt-auto">View All Goals &rarr;</a>
          </ScrollReveal>
        </div>

        {/* Records Snapshot */}
        <div className="md:col-span-1 xl:col-span-1 bg-white rounded-xl md:rounded-[14px] p-4 md:p-6 shadow-[0_4px_24px_rgba(46,125,50,0.08)] flex flex-col">
          <ScrollReveal delay={0.6} className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-3 md:mb-4">
              <h3 className="font-heading font-bold text-base md:text-lg text-dark leading-tight">Records</h3>
              <span className="text-[10px] md:text-xs text-text-muted">{userPlan.recordsUsed}/{isFree ? userPlan.recordsLimit : '∞'} used</span>
            </div>
            
            <div className="flex gap-2 mb-3 md:mb-4">
              {recentRecords.map(record => (
                <div key={record.id} className="w-8 h-8 md:w-10 md:h-10 rounded-lg bg-surface-alt border border-border flex items-center justify-center text-xs font-medium text-text-secondary" title={record.title}>
                  {record.type === 'lab_report' ? <FileText size={14} /> : record.type === 'prescription' ? <Pill size={14} /> : <FileEdit size={14} />}
                </div>
              ))}
              {recentRecords.length === 0 && <p className="text-xs md:text-sm text-text-muted">No records yet.</p>}
            </div>
            
            <a href="/records" className="text-xs md:text-sm font-medium text-secondary hover:underline mt-auto">Go to Records &rarr;</a>
          </ScrollReveal>
        </div>
